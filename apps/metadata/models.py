...
# Provides custom metadata fields, metadata history tracking,
# metadata export, and auto-fill support for EDMS documents.
# =============================================================================
...
class MetadataField(models.Model):
    """Defines a predefined or custom metadata field per DocumentType."""
    ...
    select_options = models.TextField(blank=True, help_text='Comma-separated values for SELECT type')
    is_required    = models.BooleanField(default=False)
    is_searchable  = models.BooleanField(default=True, help_text='Include in metadata search index')
    is_exportable  = models.BooleanField(default=True, help_text='Include in CSV/JSON export')
    default_value  = models.CharField(max_length=500, blank=True)
    validation_regex = models.CharField(max_length=300, blank=True,
                                        help_text='Optional regex for field validation')
    sort_order     = models.IntegerField(default=0)
    is_active      = models.BooleanField(default=True)
    created_by     = models.ForeignKey(
        settings.AUTH_USER_MODEL, null=True, blank=True,
        on_delete=models.SET_NULL, related_name='metadata_fields_created'
    )
...
class DocumentMetadata(models.Model):
    ...
    field       = models.ForeignKey(MetadataField, on_delete=models.RESTRICT, related_name='values')
    value       = models.TextField(blank=True, null=True)
    auto_filled = models.BooleanField(default=False, help_text='True if populated by OCR/AI auto-fill')
    updated_by  = models.ForeignKey(
        settings.AUTH_USER_MODEL, null=True, blank=True,
        on_delete=models.SET_NULL, related_name='metadata_updated'
    )
    updated_at  = models.DateTimeField(auto_now=True)
...
class MetadataHistory(models.Model):
    """Immutable audit history of every change to a document metadata value."""
    ...
    changed_by  = models.ForeignKey(
        settings.AUTH_USER_MODEL, null=True, blank=True,
        on_delete=models.SET_NULL, related_name='metadata_history'
    )
