...
# PRD 5.2.4 — Automatic version numbering, previous versions retained,
# version comparison, rollback to previous version with audit.
# =============================================================================
import hashlib
import difflib
...
class VersioningService:
    ...
    def next_version_number(document: Document, major: bool = False) -> str:
        ...
        latest = (
            ...
            .exclude(status=DocumentVersion.VersionStatus.DELETED)
            .order_by('-created_at')
            .first()
        )
        if not latest:
            return '1.0'
        try:
            major_n, minor_n = [int(x) for x in latest.version_number.split('.')]
        except (ValueError, AttributeError):
            return '1.0'
        return f'{major_n + 1}.0' if major else f'{major_n}.{minor_n + 1}'
    ...
    def create_version(document: Document, file_obj,
                       ...
                       source_agency: str = '',
                       change_reason: str = '',
                       probable_impacts: str = '') -> DocumentVersion:
        """Create a new version, archive previous current, compute checksum,
        and record an alteration history entry.
        """
        ...
        # Record alteration history entry (PRD Table 13.9)
        if alteration_number:
            from django.utils import timezone as tz
            AlterationHistory.objects.create(
                document=document,
                version=version,
                alteration_number=alteration_number,
                alteration_date=tz.now().date(),
                changes_description=edit_summary,
                change_reason=change_reason,
                probable_impacts=probable_impacts,
                source_agency=source_agency,
                implementation_status=AlterationHistory.ImplementationStatus.IMPLEMENTED,
                created_by=user,
            )
    ...
    def rollback(document: Document, target_version_number: str,
        ...
        Creates a new version entry (rollback trail preserved).
        PRD 5.2.4: rollback to previous version with approval.
        """
    ...
    def recover(version: DocumentVersion):
        """Recover a soft-deleted version within 30-day window."""
    ...
    def compare(version_a: DocumentVersion, version_b: DocumentVersion) -> str:
        """Generate unified diff between two versions' edit summaries."""
