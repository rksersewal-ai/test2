...
export const MonthlyKpiReportPage: React.FC = () => {
  ...
  const btnStyle: React.CSSProperties = {
    padding: '7px 16px', background: '#1e2332',
    border: '1px solid #2d3555', color: '#d1d5db',
    borderRadius: 6, fontSize: 13, cursor: 'pointer',
  };
  const primaryBtn: React.CSSProperties = {
    ...btnStyle, background: 'linear-gradient(135deg,#4b6cb7,#182848)',
    ...
    <div style={{ padding: 24 }}>
      ...
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 20, alignItems: 'flex-end' }}>
        <label style={{ color: '#94a3b8', fontSize: 12, fontWeight: 600 }}>
          ...
          <input type="number" value={year} min={2020} max={2099}
            ...
            style={{ display: 'block', marginTop: 4, width: 90, padding: '6px 10px', background: '#1e2332', border: '1px solid #2d3555', color: '#d1d5db', borderRadius: 6, fontSize: 13 }} />
        </label>
        <label style={{ color: '#94a3b8', fontSize: 12, fontWeight: 600 }}>
          Month
          <input type="number" value={month} min={1} max={12}
            onChange={e => setMonth(Number(e.target.value))}
            style={{ display: 'block', marginTop: 4, width: 70, padding: '6px 10px', background: '#1e2332', border: '1px solid #2d3555', color: '#d1d5db', borderRadius: 6, fontSize: 13 }} />
        </label>
        <label style={{ color: '#94a3b8', fontSize: 12, fontWeight: 600 }}>
          Section
          <select value={section} onChange={e => setSection(e.target.value)}
            style={{ display: 'block', marginTop: 4, padding: '6px 10px', background: '#1e2332', border: '1px solid #2d3555', color: '#d1d5db', borderRadius: 6, fontSize: 13 }}>
        ...
        <button style={primaryBtn} onClick={fetchKpi}>🔄 Load</button>
      ...
      {loading ? (
        ...
        <div style={{ background: '#151b2e', border: '1px solid #2d3555', borderRadius: 10, overflow: 'hidden' }}>
          <div style={{ padding: '12px 20px', background: '#1a2238', borderBottom: '1px solid #2d3555', color: '#4b6cb7', fontWeight: 700, fontSize: 13, letterSpacing: '0.06em' }}>
            MONTHLY WORK SUMMARY — {monthLabel}
          </div>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              <tr style={{ background: '#1e2b45' }}>
                <th style={{ padding: '10px 20px', textAlign: 'left', color: '#94a3b8', fontWeight: 600 }}>Activity</th>
                <th style={{ padding: '10px 20px', textAlign: 'right', color: '#94a3b8', fontWeight: 600 }}>Count</th>
              </tr>
            </thead>
            <tbody>
              {summary.length === 0 ? (
                <tr><td colSpan={2} style={{ padding: '20px', textAlign: 'center', color: '#4b5563' }}>No data for this period.</td></tr>
              ) : (
                summary.map(r => (
                  <tr key={r.work_category_code} style={{ borderBottom: '1px solid #1e2a3e' }}>
                    <td style={{ padding: '10px 20px', color: '#d1d5db' }}>{r.work_category_label}</td>
                    <td style={{ padding: '10px 20px', textAlign: 'right', color: '#60a5fa', fontWeight: 600 }}>{r.work_count}</td>
                  </tr>
                ))
              )}
              {summary.length > 0 && (
                <tr style={{ background: '#1a2238', borderTop: '2px solid #2d3555' }}>
                  ...
                  <td style={{ padding: '10px 20px', textAlign: 'right', color: '#34d399', fontWeight: 700, fontSize: 15 }}>{total}</td>
                </tr>
              )}
            </tbody>
          </table>
