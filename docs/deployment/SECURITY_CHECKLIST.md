...
- [ ] Set `DJANGO_ALLOWED_HOSTS` to your LAN IPs and hostname(s) e.g. `192.168.1.100,plw-edms.local`
- [ ] Set `ALLOWED_LAN_NETWORKS` to your actual office subnets e.g. `192.168.1.0/24`
- [ ] Set `DATABASE_URL` with a dedicated PostgreSQL user (not `postgres`)
...
- [ ] Set `HSTS_SECONDS=31536000` after HTTPS is confirmed stable
...
- [ ] Verify `AuditRequestMiddleware` writes to `audit_log` on POST requests
...
- [ ] Confirm OCR worker log output goes to rotating file (see `LOGGING` in settings)
...
- [ ] Annually: review and update `ALLOWED_LAN_NETWORKS` if office network changes
