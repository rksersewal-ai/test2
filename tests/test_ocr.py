...
class TestEntityParser:
    ...
    def test_extracts_plw_doc_number(self):
        entities = parse_entities('Refer to PLW/SPEC/2024/0023 for details')
    ...
    def test_extracts_rdso_spec(self):
        entities = parse_entities('As per RDSO/PE/SPEC/TL/0013 Rev-4')
        specs = [e for e in entities if e['type'] == 'SPEC']
        assert len(specs) >= 1

    def test_extracts_is_standard(self):
        entities = parse_entities('Compliance with IS:3043-2019 required')
        stds = [e for e in entities if e['type'] == 'STD']
        assert len(stds) >= 1

    def test_extracts_din_standard(self):
        entities = parse_entities('Per DIN 7168-medium tolerance')
        stds = [e for e in entities if e['type'] == 'STD']
        assert len(stds) >= 1

    def test_extracts_drawing_number(self):
        entities = parse_entities('See drawing ELW/4/3/027 Rev B')
    ...
    def test_no_false_positives_on_plain_text(self):
        entities = parse_entities('The quick brown fox jumps over the lazy dog.')
