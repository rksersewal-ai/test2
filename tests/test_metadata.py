...
# FR-005: Tests for Metadata Management module
# Covers MetadataField CRUD, DocumentMetadata set/bulk-set,
# MetadataHistory recording, CSV/JSON export, auto-fill from OCR.
# =============================================================================
import pytest
...
class TestMetadataField:
    ...
    def test_create_text_field(self):
        ...
        field = MetadataField.objects.create(
            ...
            field_type=MetadataField.FieldType.TEXT,
            is_required=True,
        )
    ...
    def test_select_field_options_list(self):
        doc_type = DocumentTypeFactory()
        field = MetadataField.objects.create(
            document_type=doc_type,
            field_name='currency',
            field_label='Currency',
            field_type=MetadataField.FieldType.SELECT,
    ...
    def test_unique_field_name_per_doc_type(self):
        ...
        with pytest.raises(IntegrityError):
            MetadataField.objects.create(
                document_type=doc_type, field_name='ref_no',
                field_label='Ref No Duplicate', field_type='text'
            )
...
class TestMetadataService:
    ...
    def test_set_value_updates_and_records_history(self):
    ...
    def test_auto_fill_from_ocr(self):
        meta = MetadataService.set_value(
            self.document, self.field, 'OCR-INV-001',
            user=self.user, auto_filled=True
        )
        assert meta.auto_filled is True

    def test_export_csv(self):
        MetadataService.set_value(self.document, self.field, 'INV-CSV-001', user=self.user)
        csv_output = MetadataService.export_csv(self.document)
        assert 'Invoice Number' in csv_output
        assert 'INV-CSV-001' in csv_output

    def test_export_json(self):
        import json
        MetadataService.set_value(self.document, self.field, 'INV-JSON-001', user=self.user)
        json_output = MetadataService.export_json(self.document)
    ...
    def test_get_history(self):
        ...
        history = MetadataService.get_history(self.document)
        assert history.count() == 1
        assert history.first().new_value == 'B'
